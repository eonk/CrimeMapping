...
#step 1: create a new object that only has the 'fallowfield' ward
df1 <- manchester_ward %>%
...
#(see week5: Activity 2, if you forget how to do!)
...
You will notice that the points in a homogeneous Poisson process are not ‘uniformly spread’: there are empty gaps and clusters of points. Also, once you display the 'quadratcount' on the plot, the number of points in each area is similar unlike the burglary pattern in Fallowfield.
Run the previous command a few times. You will see the map generated is different each time.
...
For an additional tutorial on creating hot routes in R you can read Dr Solymosi's blog post here: [rekadata.net/blog/hot-routes-tutorial/](https://rekadata.net/blog/hot-routes-tutorial/)
